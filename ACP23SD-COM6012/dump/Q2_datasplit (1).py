...
evaluator_logistic=MulticlassClassificationEvaluator(labelCol="hasClaim", metricName="accuracy")
...
best_logistic_l1_regParam = model_logistic_l1.bestModel._java_obj.getRegParam()

# Best Logistic (L2) regParam
best_logistic_l2_regParam = model_logistic_l2.bestModel._java_obj.getRegParam()
...
logistic_l1 = LogisticRegression(labelCol="hasClaim", featuresCol="features",  regParam=best_logistic_l1_regParam, elasticNetParam=1)
logistic_l2 = LogisticRegression(labelCol="hasClaim", featuresCol="features",  regParam=best_logistic_l2_regParam, elasticNetParam=0)
...
pipelineModel_logistic_l1 = pipeline.fit(train_df)
train_df_transformed_l1 = pipelineModel_logistic_l1.transform(train_df)
model_logistic_l1 = logistic_l1.fit(train_df_transformed_l1)

pipelineModel_logistic_l2 = pipeline.fit(train_df)
train_df_transformed_l2 = pipelineModel_logistic_l2.transform(train_df)
model_logistic_l2 = logistic_l2.fit(train_df_transformed_l2)



test_df_transformed = pipelineModel_poisson.transform(test_df)
test_df_transformed_l1 = pipelineModel_logistic_l1.transform(test_df)
test_df_transformed_l2 = pipelineModel_logistic_l2.transform(test_df)


predictions_poisson = model_poisson.transform(test_df_transformed)
predictions_l1 = model_logistic_l1.transform(test_df_transformed_l1)
predictions_l2 = model_logistic_l2.transform(test_df_transformed_l2)


# Evaluate models on the test set
#evaluator_poisson = RegressionEvaluator(labelCol="ClaimNb", predictionCol="prediction", metricName="rmse")

rmse_poisson = RegressionEvaluator(labelCol="ClaimNb").evaluate(predictions_poisson,{RegressionEvaluator.metricName: "rmse"})
